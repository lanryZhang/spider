 ...
 */
public class MiaoPaiDownloadJob extends AbsDownloadJob {
    private Logger log = Logger.getLogger(MiaoPaiDownloadJob.class);
    private Service service = BeanLoader.getBean("miaoPaiServiceImpl");
    @Override
    public List<VideoEntityInfo> getVideoList() {
        try{
            DownloadService service = BeanLoader.getBean("downloadServiceImpl");
            return service.getMiaoPaiList();
        }catch (Exception e){
            log.error(e);
        }
        return null;
