 ...
 */
public class YouKuDownloadJob extends AbsDownloadJob {
    private Logger log = Logger.getLogger(YouKuDownloadJob.class);
    private Service service = BeanLoader.getBean("youKuServiceImpl");
    @Override
    public List<VideoEntityInfo> getVideoList() {
        try {
            DownloadService service = BeanLoader.getBean("downloadServiceImpl");
            return service.getYouKuList();
        }catch (Exception e){
            log.error(e);
        }
        return null;
