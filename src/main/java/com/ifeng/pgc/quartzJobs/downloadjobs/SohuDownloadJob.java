 ...
 */
public class SohuDownloadJob extends AbsDownloadJob {
    private Logger log = Logger.getLogger(SohuDownloadJob.class);
    private Service service = BeanLoader.getBean("sohuServiceImpl");

    @Override
    public List<VideoEntityInfo> getVideoList() {
        try{
            DownloadService service = BeanLoader.getBean("downloadServiceImpl");
            return service.getSohuList();
        }catch (Exception e){
            log.error(e);
        }
        return null;
        ...
        try {
            ...
            if (inner != null) {
                ...
                for (int i = 0; i < inner.size(); i++) {
                    if (type.toUpperCase().equals("DIRECT")) {
