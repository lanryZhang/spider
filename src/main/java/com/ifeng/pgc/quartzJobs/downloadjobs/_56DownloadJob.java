 ...
 */
public class _56DownloadJob extends AbsDownloadJob {
    private Logger log = Logger.getLogger(_56DownloadJob.class);
    private Service service = BeanLoader.getBean("_56serviceImpl");
    @Override
    public List<VideoEntityInfo> getVideoList() {
        try {
            DownloadService service = BeanLoader.getBean("downloadServiceImpl");
            return service.get56List();
        }catch (Exception e){
            log.error(e);
        }
        return null;
