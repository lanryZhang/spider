...
import com.ifeng.pgc.beans.MiaoPaiPageInfo;
 ...
 */
public class MiaoPaiPageJob extends AbsHtmlJob {

    private Service<VideoEntityInfo> service;
    private Logger log = Logger.getLogger(MiaoPaiPageJob.class);
    public MiaoPaiPageJob() {
        super(MiaoPaiPageInfo.class);
    }
    ...
    public void doExecute(Context context) {
        ...
        try {
            service = BeanLoader.getBean(serviceName);
            MiaoPaiPageInfo temp = item.get(MiaoPaiPageInfo.class.getName());
            if (temp != null) {
                VideoEntityInfo mp = new VideoEntityInfo();
                mp.setKeywords(temp.getKeywords());
                ...
                mp.setTitle(temp.getTitle());
                mp.setUpdateStatus(1);
                //if (mp.getComments() != null)
                    service.update(mp);
            }
        } catch (Exception e) {
