...
import com.ifeng.pgc.beans.JinRiTouTiaoVideoPageInfo;
 ...
 */
public class JinRiTouTiaoPageJob extends AbsHtmlJob {

    private Service<VideoEntityInfo> service;
    private Logger log = Logger.getLogger(JinRiTouTiaoPageJob.class);
    public JinRiTouTiaoPageJob() {
        super(JinRiTouTiaoVideoPageInfo.class);
    }
     @Override
    ...
    public void doExecute(Context context) {
        ...
        try {
            service = BeanLoader.getBean(serviceName);
            JinRiTouTiaoVideoPageInfo temp = item.get(JinRiTouTiaoVideoPageInfo.class.getName());
            if (temp != null) {
                VideoEntityInfo mp = new VideoEntityInfo();
                String keywords = "";
                if (temp.getKeywords()!= null && temp.getKeywords().size() > 0) {
